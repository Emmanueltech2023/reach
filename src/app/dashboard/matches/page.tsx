...
export default function MatchesPage() {
  ...
  const fetchData = useCallback(async (forceRefresh = false) => {
    try {
      ...
      const res = await fetch(`/api/ai/matches?investorId=${user.id}&limit=20`);
  ...
  // Wait for tier to load before deciding what to show
  if (tierLoading) {
    return (
  ...
  return (
    <DashboardShell role="investor" fullName={profile?.full_name} username={profile?.username}>
      <div className="max-w-3xl mx-auto flex flex-col gap-5">
        ...
        {/* TIER GATE — blocks free users */}
        {!features.canAccessAIMatches ? (
          <UpgradeGate
            requiredTier="pro"
            currentTier={features.tier}
            featureName="AI Match Engine"
            featureDesc="Get scored startup recommendations matched to your investment thesis. Upgrade to Pro to unlock AI-powered matching with reasons like sector fit, ticket size, and trust signals."
          >
            <div />
          </UpgradeGate>
        ) : (
                    ...
                    >
                      <div className="flex items-start gap-3 mb-3">
                        <div className="flex-shrink-0 w-6 h-6 flex items-center justify-center">
                          <span className={`font-bold ${index < 3 ? "text-[#C9A84C] text-sm" : "text-[#5C5A70] text-xs"}`}>
                            #{index + 1}
                          </span>
                        </div>

                        {match.logo_url ? (
                          <img src={match.logo_url} alt={match.name}
                            className="w-10 h-10 rounded-lg object-cover flex-shrink-0" />
                        ) : (
                          <div className="w-10 h-10 rounded-lg bg-[#C9A84C20] flex items-center justify-center flex-shrink-0">
                            <span className="text-sm font-medium text-[#C9A84C]">{match.name?.[0]}</span>
                          </div>
                        )}
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-start justify-between gap-2">
                            <div>
                              <div className="flex items-center gap-2">
                                <h3 className="text-[#F5F3ED] text-sm font-medium truncate">{match.name}</h3>
                                {match.tier === "premium" && (
                                  <Star size={11} className="text-[#C9A84C] flex-shrink-0" fill="#C9A84C" />
                                )}
                              </div>
                              <div className="flex items-center gap-1 mt-0.5">
                                ...
                                <span className="text-[#5C5A70] text-xs">{match.profiles?.full_name}</span>
                              </div>
                            </div>

                            <div className={`flex flex-col items-center px-2.5 py-1.5 rounded-lg border flex-shrink-0 ${scoreBg}`}>
                              <span className={`text-base font-bold ${scoreColor}`}>{match.match_score}</span>
                          ...
                          <div className="flex items-center gap-2 mt-2 flex-wrap">
                            <span className={`text-xs px-2 py-0.5 rounded-full ${
                              match.category === "web3" ? "bg-purple-900 text-purple-300" : "bg-blue-900 text-blue-300"
                            }`}>
                      ...
                      <div className="flex gap-3 mb-3">
                        <div className="flex-1">
                          ...
                          <div className="text-[#F5F3ED] text-sm font-medium">{formatCurrency(match.funding_goal)}</div>
                        </div>
                        <div className="flex-1">
                          <div className="text-[#5C5A70] text-xs mb-0.5">Equity</div>
                          <div className="text-[#F5F3ED] text-sm font-medium">{match.equity_offered}%</div>
                        </div>
                        <div className="flex-1">
                      ...
                      <div className="h-1.5 bg-[#2A2A3E] rounded-full overflow-hidden mb-3">
                        <div className="h-full bg-[#C9A84C] rounded-full" style={{ width: `${raisedPct}%` }} />
                      </div>
